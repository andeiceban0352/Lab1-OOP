...
void reverse(struct node **head){
    if(head == NULL) {
    	printf("Adaugati valori pentru a face reversie :)) ");
    }else {
    	struct node *tail = *head;
    	while(tail->next != NULL) {
    	    tail = tail->next;
...
void backwards_traversal(struct node **head) {
    struct node *tail = *head;
...
int main() {
    ...
    while(number != 15) {
        ...
        printf("- 13 Save data in the file \n");
        printf("\n");
